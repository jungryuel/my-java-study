...
CircularLinkedList(NodeBi node){
	this.head = node;
	this.tail = node;
	node.next = this.head;//원형이니까 자기 자신으로 순환
	node.prev = this.tail;//원형이니까 자기 자신으로 순환
}
...
public void addData(int data,Integer beforeData) {
	...
	NodeBi newNodeBi =new NodeBi(data,this.head,this.tail);//다음은 헤드 이전은 테일
	this.tail = newNodeBi;
	this.tail.next = newNodeBi;
	this.tail.prev = newNodeBi;
	}else {
		...
		do {
			if(cur.data == beforeData) {
				if(cur == this.head) {
					NodeBi newNodeBi =new NodeBi(data,this.head,this.tail);					this.tail = newNodeBi;
					this.tail.next = newNodeBi;
					this.tail.prev = newNodeBi;
				}else {
...
public void showData() {
	...
	System.out.println(cur.data);//가장 끝 노드의 데이터 출력
}
