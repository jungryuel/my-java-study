...
public class zerobase8 {
	...
	public static void main(String[] args) {
        ...
        for (int i = 0; i < myArray2.length; i++) {//배열 전체의 길이는 행
            for (int j = 0; j < myArray2[i].length; j++) {//i의 길이는 열
...
//          모든 원소를 1로 변경하고, 대각 원소는 10으로 변경하시오.
        ...
        for(int i = 0; i < testArray1.length; i++){
        	for(int j = 0; j < testArray1[i].length; j++){
        		testArray1[i][j]=1;//모든원소를 1로 초기화
        		
        		if(i==j) {
        			testArray1[i][j]=10;//대각원소는 인덱스의 값이 일치
        		}
        	}
